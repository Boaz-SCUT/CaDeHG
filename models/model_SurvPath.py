...
class SurvPath(nn.Module):
    def __init__(
        ...
        self.to_logits0 = nn.Sequential(
                nn.Linear(self.wsi_projection_dim*self.num_pathways, self.num_pathways),
                nn.ReLU(),
                nn.Linear(self.num_pathways, self.num_classes)
            )
    ...
    def forward(self, **kwargs):
        ...
        h_omic_bag = torch.stack(h_omic).unsqueeze(0)  ### [1, num_pathways=331, dim=256] omic embeddings are stacked (to be used in co-attention)
        
        #---> project wsi to smaller dimension (same as pathway dimension)
        wsi_embed = self.wsi_projection_net(wsi) # [1, 4096, 256]
        # print('wsi_embed.shape:', wsi_embed.shape)
        ...
        #---> get logits
        logits = self.to_logits(embedding) # [1, 4]
