...
def _prepare_for_experiment(args):
    ...
    assert os.path.isdir(args.split_dir) # 'splits/5foldcv/tcga_blca'
...
def _get_custom_exp_code(args):
    ...
    # -----> ccd_alpha
    # param_code += ('_ccd_alpha' + str(args.alpha_ccd))
    
    # ----->  _beta
    # param_code += ('_ccd_beta' + str(args.beta_ccd))
    
    

    #----> Regularization
    # if args.reg_type == 'L1':
    #   # It appears that the code snippet is using a placeholder "param_code" and does not contain
    # any actual code. Without the actual code inside the snippet, it is not possible to determine
...
def _collate_survpath(batch):
    ...
    # 如果长度是8, 则有raw_gene_tensor
    if len(batch[0]) == 8:
        raw_gene_tensor = torch.stack([item[1] for item in batch])
        omic_data_list = []
        for item in batch:
        ...
        return [img, raw_gene_tensor, omic_data_list, label, event_time, c, clinical_data_list, mask]
    
    else:
        raw_gene_tensor = None
        omic_data_list = []
        for item in batch:
...
def _collate_ccd_survpath(batch):
    r"""
    Collate function for survpath
...
def _get_split_loader(args, split_dataset, training = False, testing = False, weighted = False, batch_size=1):
    ...
    elif args.modality in ["survpath", "double_mediator_survpath", "ccd_survpath"]:
         collate_fn = _collate_survpath 
    elif args.modality in ["ccd_survpath"]:
        collate_fn = _collate_ccd_survpath
    else:
