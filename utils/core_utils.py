...
def _step(cur, args, loss_fn, model, optimizer, scheduler, train_loader, val_loader):
    ...
    for epoch in range(args.max_epochs):
        ...
        print('Val loss:', total_loss, ', best val_c_index:', best_cindex)
    ...
    print('Final Val c-index: {:.4f}'.format(best_cindex))
    ...
    return results_dict, (best_cindex, val_cindex_ipcw, val_BS, val_IBS, val_iauc, total_loss)
...
def _summary_ccd00(dataset_factory, model, modality, loader, loss_fn, survival_train=None, use_counterfactual=True):
    """
    CCD模型的评估函数，支持反事实推理
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def _summary_ccd(dataset_factory, model, modality, loader, loss_fn, survival_train=None, use_counterfactual=True):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(loader, desc="Validation Progress")):
            try:
                # 每10个batch清理一次显存
                # if batch_idx % 10 == 0:
                #     torch.cuda.empty_cache()
                #     if batch_idx % 100 == 0:  # 每50个batch打印一次状态
                #         print_memory_status(f"Batch {batch_idx}")
                ...
                # 清理batch相关的张量
                del h, data_WSI, raw_gene_expr, data_omics
...
def _train_val_ccd(datasets, cur, args):
    try:
        ...
        # 确保在函数结束时清理所有对象
        locals_to_delete = ['model', 'optimizer', 'train_loader', 'val_loader', 
                           'lr_scheduler', 'train_split', 'val_split']
