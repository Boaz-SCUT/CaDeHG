...
class SurvivalDatasetFactory:
    ...
    def _clean_label_data(self):
        ...
        if "IDC" in self.label_data['oncotree_code'].values: # must be BRCA (and if so, use only IDCs)
        ...
        if self.enable_multitask:
            ...
            # Debug: check data formats before merging
            # if self.print_info:
            #     print("\n--- DEBUG: Data format analysis ---")
            #     print(f"Label data case_id samples: {self.label_data['case_id'].head().tolist()}")
            #     print(f"Clinical data index samples: {self.clinical_data.index[:5].tolist()}")
            #     print(f"Label data case_id type: {self.label_data['case_id'].dtype}")
            #     print(f"Clinical data index type: {self.clinical_data.index.dtype}")
            ...
            # if self.print_info:
            #     print(f"Label data unique case_ids: {len(label_case_ids)}")
            #     print(f"Clinical data unique case_ids: {len(clinical_case_ids)}")
            #     print(f"Overlapping case_ids: {len(overlap)}")
            #     if len(overlap) > 0:
            #         print(f"Found direct overlap! Sample overlapping case_ids: {list(overlap)[:5]}")
            #     else:
            #         print("No overlap found! Checking if case_ids need truncation...")
            #         # Check if label case_ids are longer (like TCGA-XX-XXXX-01Z-00-DX1...)
            #         label_sample = list(label_case_ids)[0] if label_case_ids else ""
            #         clinical_sample = list(clinical_case_ids)[0] if clinical_case_ids else ""
            #         print(f"Sample label case_id: '{label_sample}' (length: {len(label_sample)})")
            #         print(f"Sample clinical case_id: '{clinical_sample}' (length: {len(clinical_sample)})")
            ...
            if self.print_info:
                ...
                print(f"Non-null stage values: {merged_data['stage'].notna().sum()}")
                print(f"Stage value counts after merge:")
                print(merged_data['stage'].value_counts(dropna=False))
            ...
            if self.print_info:
                print(f"Filtered out {initial_count - final_count} samples without valid stage information")
                print(f"Remaining samples: {final_count}")
                if final_count == 0:
                    raise ValueError("No samples remaining after filtering for valid stage information!")
