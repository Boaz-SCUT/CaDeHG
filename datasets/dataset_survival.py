...
class SurvivalDatasetFactory:
    ...
    def _load_clinical_data(self):
        ...
        """
        path_to_data = "./datasets_csv/clinical_data/{}_clinical1.csv".format(self.study)
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #@TODO what is the difference between tmil_abmil and transmil_wsi
        ...
        elif self.modality in ["survpath", "double_mediator_survpath"]:
            if self.modality in ["survpath", "ccd2_survpath"]:
                patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            else:
                patch_features = None
                mask = None
        ...
        elif self.modality == 'ccd_survpath': # 改.......
            # 获取原始基因表达数据      
            df_small = self.omics_data_dict["rna"][self.omics_data_dict["rna"]["temp_index"] == case_id]
            df_small = df_small.drop(columns="temp_index")
            df_small = df_small.reindex(sorted(df_small.columns), axis=1)
            raw_gene_tensor = torch.squeeze(torch.Tensor(df_small.values))  # 原始基因数据
            ...
            return (patch_features, raw_gene_tensor, omic_list, label, event_time, c, clinical_data, mask)
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids):
        ...
        for slide_id in slide_ids:
            ...
            # except:
                # 将不存在的病理图片ID打印出来
                # print(f"Warning: Slide ID {slide_id} not found at path {wsi_path}. Skipping this slide.")
                # patch_features.append(torch.zeros((0, 1024)))  # Assuming 1024 is the feature dimension
                # continue
